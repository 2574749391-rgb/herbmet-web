...
V2.0 将产品定位升级为中药材代谢研究智能体平台：首页首位提供无需预先检索的快速问答，两阶段研究作为专业证据工具保留，报告内继续提供基于当前证据的问答。界面明确区分快速模型回答与经过文献检索的证据回答。

V2.1 增加智能体研究意图识别：对话涉及已收录药材的文献、证据或 ADME 问题时，可在对话区确认并由智能体直接执行第一阶段检索，再进入候选成分确认流程。
